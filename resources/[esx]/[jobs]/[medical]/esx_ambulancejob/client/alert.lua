     ...
     while true do
		...
		if AppelEnAttente then
			...
			elseif IsControlJustPressed(1, 73) and AppelEnAttente then
				ESX.ShowAdvancedNotification("EMS", "~b~EMS Requested", "You have rejected the call.", "CHAR_CALL911", 8)
		...
		if IsControlJustPressed(1, 246) and AppelDejaPris == true then
			ESX.ShowAdvancedNotification("EMS", "~b~EMS Requested", "The call has been answered.", "CHAR_CALL911", 8)
		end
     end
end)
...
AddEventHandler("emsAppelPris", function(Xid, XAppelCoords)
	ESX.ShowAdvancedNotification("EMS", "~b~EMS Requested", "You have taken the call, follow the GPS.", "CHAR_CALL911", 8)   
	...
	-- Ajout de la route
     ...
     while rea do
          if GetDistanceBetweenCoords(XAcoords, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
               ...
               Draw3DText(XAcoords.x, XAcoords.y, XAcoords.z, "~g~A person needs to be revived here ...\n~b~[E]~w~to revive.", 4, 0.1, 0.1)
               ...
               if IsControlJustReleased(0, 38) then
				...
				TriggerServerEvent('esx_ambulancejob:revive', XAid)
				    TriggerEvent('mythic_hospital:client:RemoveBleed', XAid)
    				TriggerEvent('mythic_hospital:client:ResetLimbs', XAid)
