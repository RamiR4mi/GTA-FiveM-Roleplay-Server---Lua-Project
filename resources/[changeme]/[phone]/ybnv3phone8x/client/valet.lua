...
RegisterNUICallback('getCarsValet', function(data)
    ...
    ESX.TriggerServerCallback('ybnv3phone8x:loadVehicle', function(vehicle)

    local props = json.decode(vehicle[1].vehicle)

    if enroute then
		TriggerEvent('ybnv3phone8x:notifi', {title = 'Vale', message = _U('vale_gete'), img= '/html/static/img/icons/vale.png' })
        return
    end
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
            if ESX.Math.Trim(GetVehicleNumberPlateText(vehicle)) == ESX.Math.Trim(props.plate) then
				...
				TriggerEvent('ybnv3phone8x:notifi', {title = 'Vale', message = _U('vale_getr'), img= '/html/static/img/icons/vale.png' })
				return
			end
	...
	ESX.TriggerServerCallback('ybnv3phone8x:checkMoney2', function(hasEnoughMoney)
		if hasEnoughMoney == true then
			SpawnVehicle(props, props.plate)
			TriggerServerEvent('ybnv3phone8x:finish')
		else

			TriggerEvent('ybnv3phone8x:notifi', {title = 'Vale', message = _U('vale_checmoney'), img= '/html/static/img/icons/vale.png' })
...
function SpawnVehicle(vehicle, plate)
    ...
    while not HasModelLoaded(driverhash) and RequestModel(driverhash) or not HasModelLoaded(vehhash) and RequestModel(vehhash) do
  	...
  	if found and HasModelLoaded(driverhash) then
		...
		--TriggerServerEvent('ybnv3phone8x:valet-car-set-outside', plate)
		valeOn = true
