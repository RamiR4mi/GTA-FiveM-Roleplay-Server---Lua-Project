...
if Config.ESXVersion == '1.2' then
  ...
  RegisterServerEvent('ybnv3phone8x:transferPhoneNumber')
  AddEventHandler('ybnv3phone8x:transferPhoneNumber', function(to, totaltt, tfee)
      ...
      if zPlayer ~= nil then
          ...
          if xPlayer.identifier == zPlayerIden then
              ...
              TriggerClientEvent('ybnv3phone8x:notifi', _source, {title = 'Bank', message = _U('bank_yourself'), img= '/html/static/img/icons/wallet.png' })

          else
              if balance <= 0 or balance < tonumber(TotalMoney) or tonumber(TotalMoney) <= 0 then
                  -- advanced notification with bank icon

                  TriggerClientEvent('ybnv3phone8x:notifi', _source, {title = 'Bank', message = _U('bank_nomoney'), img= '/html/static/img/icons/wallet.png' })
                  ...
                  TriggerClientEvent('ybnv3phone8x:notifi', _source, {title = 'Bank', message = name2 .. _U('bank_transfer'), img= '/html/static/img/icons/wallet.png' })
                  TriggerClientEvent('ybnv3phone8x:notifi', zPlayer.source, {title = 'Bank', message = _U('bank_playertransfer') ..name, img= '/html/static/img/icons/wallet.png' })
                  ...
                  BankGetBilling(_source, function (bankkkkk)
                    TriggerClientEvent('ybnv3phone8x:bank_getBilling', _source, bankkkkk)
                  end)
        ...
        MySQL.Async.fetchAll("SELECT accounts FROM users WHERE identifier = @identifier", {
          ...
          TriggerClientEvent('ybnv3phone8x:notifi', _source, {title = 'Bank', message = _U('bank_systemnophone'), img= '/html/static/img/icons/wallet.png' })
  ...
  RegisterServerEvent('ybnv3phone8x:aracisatt')
  AddEventHandler('ybnv3phone8x:aracisatt', function(a, b, c)
  ...
  if zPlayer ~= nil then
    ...
    if bidentifier == zPlayer.identifier then
      TriggerClientEvent('ybnv3phone8x:notifi', src, {title = 'Car Sallers', message = _U('carseller_ownbuy'), img= '/html/static/img/icons/carsales.png' })
    else
      
      if balance < tonumber(c) then
  
        TriggerClientEvent('ybnv3phone8x:notifi', src, {title = 'Car Sallers', message = _U('carseller_nobank'), img= '/html/static/img/icons/carsales.png' })
      else
  
  
  
          TriggerClientEvent('ybnv3phone8x:cardel', -1, b)
          satici.removeAccountMoney('bank', tonumber(c))
          zPlayer.addAccountMoney('bank', tonumber(c))
  
          TriggerClientEvent('ybnv3phone8x:notifi', src, {title = 'Car Sallers', message = _U('carseller_buyvehicle'), img= '/html/static/img/icons/carsales.png' })
          TriggerClientEvent('ybnv3phone8x:notifi', zPlayer.source, {title = 'Car Sallers', message = _U('carseller_soldvehicle'), img= '/html/static/img/icons/carsales.png' })
          ...
          MySQL.Async.execute('DELETE FROM ybnv3phone8x_vehicle_sales WHERE owner = @owner AND plate = @plate', {
            ['@owner'] = a,
            ['@plate'] = b,
          }, function ()
            MySQL.Async.fetchAll('SELECT * from ybnv3phone8x_vehicle_sales ORDER BY TIME DESC LIMIT 30', {}, function (tweets)
          ...
          TriggerEvent('ybnv3phone8x:carsellernew', a, name2, bidentifier, name, b, c)
...
function BankGetBilling (accountId, cb)
  local xPlayer = ESX.GetPlayerFromId(accountId)
    MySQL.Async.fetchAll([===[
      SELECT * FROM ybnv3phone8x_bank_transfer WHERE identifier = @identifier ORDER BY time DESC LIMIT 10
      ]===], { ['@identifier'] = xPlayer.identifier }, cb)
  end 
  
  

RegisterServerEvent('ybnv3phone8x:bank_getBilling')
AddEventHandler('ybnv3phone8x:bank_getBilling', function()
  local src = source
    BankGetBilling(src, function (bankkkkk)
      TriggerClientEvent('ybnv3phone8x:bank_getBilling', src, bankkkkk)
    end)
end)


ESX.RegisterServerCallback('ybnv3phone8x:isparasicek', function(source, cb)
