...
AddEventHandler("Farmcyc:recoleccion", function()
	...
	local camarones = xPlayer.getInventoryItem('camarones').count	
         ...
         if recoleccionsuerte < 50 then
         	...
         	TriggerClientEvent('esx:showNotification', source, '~r~you can not carry more crabs')
else 
         ...
         elseif recoleccionsuerte > 51 then 
         	...
         	TriggerClientEvent('esx:showNotification', source, '~r~you can not carry more shrimps')
else 
...
ESX.RegisterServerCallback('Farmcyc:empaquetado:cangrejos', function (source, cb)
	...
	local xPlayer  = ESX.GetPlayerFromId(_source)
			...
			TriggerClientEvent('esx:showNotification', source, '~r~You can not carry more crab boxes')
				...
				TriggerClientEvent('esx:showNotification', source, '~r~You do not have crabs')
				cb(false)
				end
...
ESX.RegisterServerCallback('Farmcyc:empaquetado:camarones', function (source, cb)
	...
	local xPlayer  = ESX.GetPlayerFromId(_source)
			...
			TriggerClientEvent('esx:showNotification', source, '~r~You can not carry more shrimp boxes')
				...
				TriggerClientEvent('esx:showNotification', source, '~r~You do not have shrimps')
				cb(false)
				end
...
elseif camarones < 1 then
	TriggerClientEvent('esx:showNotification', source, '~r~You have no shrimps to sell')
